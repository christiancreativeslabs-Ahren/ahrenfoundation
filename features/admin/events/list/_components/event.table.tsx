...
import { useEventList } from "../_hooks/use-event-list";
import { buildEventColumns } from "./event.columns";
import { EventCard } from "./event.card";
import { EventFilterSheet, type EventListFilterState } from "./event.filter.sheet";

interface EventTableProps {
  initialData: EventListResponse;
  initialFilters: EventListInput;
}
...
export function EventTable({ initialData, initialFilters }: EventTableProps) {
    ...
    <div className="space-y-6">
      <Card className="border-white/10 bg-white/[0.03] text-white">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              ...
              <CardDescription className="max-w-2xl text-sm leading-relaxed text-slate-300">
                Fast cursor listing for events, using the same shared table pattern as join applications.
              </CardDescription>
            </div>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-3 xl:grid-cols-5">
          {[
            { label: "Total events", value: summary?.total ?? data?.totalCount ?? initialData.totalCount },
      ...
      <Card className="border-white/10 bg-white/[0.03] text-white">
        <CardHeader className="pb-4">
          <CardTitle className="text-lg font-semibold">Event queue</CardTitle>
          <CardDescription className="text-slate-300">
            Showing the current event queue with cursor pagination and URL-backed filters.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <DataTableToolbar
            table={table}
            title={<h2 className="text-2xl font-bold">Events [{formatCount(data?.totalCount ?? initialData.totalCount)}]</h2>}
            searchValue={currentInput.search || ""}
            onSearchChange={(value) => updateFilters({ search: value || null })}
            searchPlaceholder="Search title, summary, location..."
      ...
      <EventFilterSheet
        ...
        onApply={(filters: EventListFilterState) =>
          updateFilters({

...
type JoinApplicationActionsProps = {
...
export function JoinApplicationActions({
  application,
  mentors,
  onViewDetails,
  className,
}: JoinApplicationActionsProps) {
    ...
    <div className={cn("relative inline-flex", className)}>
          ...
          <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-56 overflow-hidden rounded-2xl border border-white/10 bg-[#07102c] p-2 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
            <button
              ...
              onClick={(event) => {
                event.stopPropagation();
                setMenuOpen(false);
                onViewDetails(application.joinApplicationId);
              }}
            >
            ...
            {application.applicationType === "youth" && application.programMemberId ? (
              <button
                type="button"
                className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-white transition hover:bg-white/[0.08]"
                onClick={(event) => {
      ...
      {application.applicationType === "youth" && application.programMemberId ? (
        <MentorAssignmentDialog
          memberId={application.programMemberId}

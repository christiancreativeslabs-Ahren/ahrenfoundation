...
type ResendWorkbookEmailDialogProps = {
  deliveries: WorkbookDeliveryOption[];
  resendAction: (formData: FormData) => void | Promise<void>;
  disabled?: boolean;
};

export function ResendWorkbookEmailDialog({
  deliveries,
  resendAction,
  disabled = false,
}: ResendWorkbookEmailDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Resend a Workbook email</DialogTitle>
          <DialogDescription>
            Choose one Workbook delivery to resend. This will send only the selected module email.
          </DialogDescription>
        </DialogHeader>
        ...
        >
            ...
            <div className="grid gap-3 rounded-xl border border-white/10 bg-white/5 p-4 text-sm text-slate-200 md:grid-cols-3">
              ...
              <div className="md:col-span-3">
                <Badge variant="outline" className="border-white/15 text-white">
                  One Workbook email will be resent
                </Badge>
              </div>
            </div>
          ) : null}
          ...
          <DialogFooter>
            ...
            <Button type="submit" className="gap-2 bg-gradient-to-r from-[#00c9ff] to-[#00ff9d] text-[#080d2e]">
              ...
              Resend selected Workbook email
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>

...
export function emailShell(input: {
  ...
  bodyHtml: string;
  openPixelUrl?: string;
  showWorkbookMap?: boolean;
}) {
  const showWorkbookMap = input.showWorkbookMap ?? true;
          ...
          <td style="padding:20px 12px 28px;">
            <table role="presentation" style="width:100%;max-width:720px;margin:0 auto;border-collapse:separate;border-spacing:0;background-color:${colors.panel};border:1px solid ${colors.border};border-radius:32px;overflow:hidden;box-shadow:0 24px 60px rgba(15,23,42,0.08);">
              <tbody>
                ...
                <tr>
                  <td colspan="${showWorkbookMap ? "2" : "1"}" style="padding:18px 32px 0;">
                    <table role="presentation" style="width:100%;border-collapse:separate;border-spacing:0;background:linear-gradient(135deg, ${colors.softSky}, ${colors.panel} 42%, ${colors.softMint});border:1px solid ${colors.border};border-radius:28px;overflow:hidden;">
                      <tbody>
                        <tr>
                          <td style="vertical-align:top;padding:22px 24px 24px;width:${showWorkbookMap ? "66%" : "100%"};"
                          >
                          
                            <p style="margin:0 0 12px;color:${colors.muted};font-size:10px;font-weight:800;letter-spacing:2px;text-transform:uppercase;">
                              ${escapeHtml(input.heroEyebrow)}
                            </p>
                            <h1 style="margin:0 0 14px;color:${colors.black};font-size:38px;font-weight:800;line-height:42px;letter-spacing:-1.6px;max-width:${showWorkbookMap ? "12ch" : "20ch"};font-family:Georgia, 'Times New Roman', serif;">
                              ${escapeHtml(input.heroTitle)}
                            </h1>
                            ${
                              input.heroSubtitle
                                ? `<p style="margin:0;color:${colors.ink};font-size:16px;font-weight:400;line-height:29px;max-width:${showWorkbookMap ? "34em" : "54em"};">${escapeHtml(input.heroSubtitle)}</p>`
                                : ""
                            }
                          </td>
                            ${
                                    ...
                                    <table role="presentation" style="width:100%;border-collapse:collapse;">
                                      <tbody>
                                        <tr>
                                          ...
                                          <td style="padding:0 0 10px;color:${colors.text};font-size:13px;line-height:20px;">Read the Workbook module and scripture with focus.</td>
                                        </tr>
                                        <tr>
                        ...
                        <tr>
                          <td style="padding-top:18px;vertical-align:top;width:60%;">
                            ...
                            <p style="margin:0;color:${colors.faint};font-size:11px;line-height:18px;max-width:30em;">
                              Every Workbook module is a step toward stronger character, sharper craft, and clearer calling.
                            </p>
                          </td>

...
import { moduleDefinitionFromPayload } from "@/lib/workbook/email-shared";
import {
  buildEmailOpenUrl,
  buildTrackedWorkbookClickUrl,
} from "@/lib/workbook/urls";
export {
  buildEmailOpenUrl,
...
export async function enrollMemberInWorkbookProgram(
  programMemberId: string,
  signedUpAt = new Date()
) {
  const { program, modules } = await ensureAhrenWorkbookProgram();
  ...
  if (member.role !== "youth") {
    throw new Error(
      "Only youth members can be synced into Workbook module deliveries."
    );
  }
...
export async function syncMemberWorkbookDeliveries(programMemberId: string) {
  const [member] = await db
    .select()
    .from(programMembers)
    .where(eq(programMembers.id, programMemberId))
  ...
  if (!enrollments.length) {
    await enrollMemberInWorkbookProgram(programMemberId, member.createdAt);
    ...
    return { created: AHREN_WORKBOOK_PROGRAM.modules.length, enrollments: 1 };
  }
...
export async function sendModuleDeliveryNow(
  ...
  const workbookModuleUrl = buildTrackedWorkbookClickUrl(
    row.delivery.id,
...
export async function submitWorkbookDeliveryAnswers(input: {
  deliveryId: string;
  accessToken: string;
  ...
  if (!row) {
    throw new Error("This Workbook link is invalid or has expired.");
  }
